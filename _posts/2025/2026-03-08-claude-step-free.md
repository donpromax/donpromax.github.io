...
| 特性 | 说明 |
|------|------|
| **模型类型** | Step-3.5-Flash 是 Step 系列的高效推理版本 |
| **总参数量** | 196.81B (激活参数约 11B) |
...
这个模型采用 MoE(Mixture of Experts)架构,虽然总参数量达 196.81B,但推理时仅激活约 11B 参数,因此在保持高质量输出的同时,实现了更快的响应速度和更低的计算成本。在日常编程任务中表现优秀,足够满足大部分代码辅助需求。
